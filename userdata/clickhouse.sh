...
cat > /root/clickhouse-configurations/config.xml <<'EOF'
<!--
...
<clickhouse>
    ...
    <!-- Let's Encrypt certificate provider configuration -->
    <!--
    <acme>
    ...
    <!-- Workload scheduling: if enabled during CPU overloaded periods, long-running queries will downscale to lower number of threads dynamically.
         Ensures more fair CPU resource distribution. (Enabled by default)
    -->
    <!--
    ...
    <!-- Settings to tune cpu and memory overload warnings for system.warnings.
